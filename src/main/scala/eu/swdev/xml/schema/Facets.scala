...
object FacetsMod {
  ...
  case class Facets[X](underlying: Map[Facet, FacetValue[X, _]]) {
  ...
  object Facets {
    ...
    case class FacetOp[F <: Facet, X, V, FV <: FacetValue[X, V]](facets: Facets[X], facet: F, createFacetValue: V => FV) {
    ...
    implicit class OrderFacetsOps[X](facets: Facets[X])(implicit ev: Ordering[X]) {
    ...
    implicit class LengthFacetsOps[X](facets: Facets[X])(implicit ev: HasLength[X]) {
    ...
    implicit class DigitFacetsOps[X](facets: Facets[X])(implicit ev: HasDigits[X]) {
    ...
    implicit class ExplicitTimeZoneOp[X](facets: Facets[X])(implicit ev: HasTimeZone[X]) {
    ...
    implicit class GeneralFacetsOps[X](facets: Facets[X]) {
    ...
    object HasLength {
      ...
      implicit def listHasLength[X] = new HasLength[List[X]] {
        override def length(x: List[X]): Int = x.size
      }

      implicit def stringHasLength = new HasLength[String] {
